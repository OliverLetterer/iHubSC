...
- (void)tableView:(UIExpandableTableView *)tableView downloadDataForExpandableSection:(NSInteger)section {
    if (section == kUITableViewControllerSectionInfoOpenIssues) {
        [GHAPIIssueV3 issuesOnRepository:self.repository milestone:self.milestoneNumber state:@"open" page:1 
        ...
        [GHAPIIssueV3 issuesOnRepository:self.repository milestone:self.milestoneNumber state:@"closed" page:1 
                       completionHandler:^(NSArray *issues, NSInteger nextPage, NSError *error) {
                           if (error) {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == kUITableViewControllerSectionInfoOpenIssues && indexPath.row == [self.openIssues count] && indexPath.row != 0 && _openIssuesNextPage > 1) {
        [GHAPIIssueV3 issuesOnRepository:self.repository milestone:self.milestoneNumber state:@"open" page:_openIssuesNextPage 
                       completionHandler:^(NSArray *issues, NSInteger nextPage, NSError *error) {
                           if (error) {
                               [self handleError:error];
                           } else {
        ...
        [GHAPIIssueV3 issuesOnRepository:self.repository milestone:self.milestoneNumber state:@"closed" page:_closedIssuesNextPage 
                       completionHandler:^(NSArray *issues, NSInteger nextPage, NSError *error) {
                           if (error) {
                               [self handleError:error];
                           } else {
